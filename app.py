...
MOCK_MOVIES = {
    'Sci-Fi': [
        ...
        ('The Matrix', 8.7, 'English'),
        ('Interstellar', 8.6, 'English'),
        ('Blade Runner 2049', 8.0, 'English'),
        ('Tenet', 7.3, 'English'),
        ('Dune', 8.0, 'English'),
        ...
        ('Avatar', 7.8, 'English'),
        ('Minority Report', 7.6, 'English'),
        ('Total Recall', 7.5, 'English'),
        ('Elysium', 6.6, 'English'),
        ...
        ('The Batman', 7.9, 'English'),
        ('Spider-Man: Across the Spider-Verse', 8.4, 'English'),
        ('Everything Everywhere All at Once', 8.1, 'English'),
        ('Brahmastra', 5.8, 'Hindi'),
        ('KGF Chapter 1', 8.2, 'Kannada'),
    ...
    'Sci-Fi-Latest-Extras': [
        ('Dune: Part Two', 8.2, 'English'),
        ('The Creator', 6.5, 'English'),
        ('Avatar: The Way of Water', 7.6, 'English')
    ],
    'Action': [
        # English
        ('The Dark Knight', 9.0, 'English'),
        ('Mad Max Fury Road', 8.1, 'English'),
        ('John Wick', 7.4, 'English'),
...
def use_mock_recommender(genre, rating, language=None):
    ...
    filtered = []
    for title, score, lang in MOCK_MOVIES[genre]:
        if score >= rating:
            # If language is specified and not 'All', filter by language
            if language and language != 'All Languages':
                if lang == language:
                    filtered.append({'title': title, 'score': score, 'language': lang})
            else:
...
def search_movie():
    ...
    try:
        ...
        for genre, movies in MOCK_MOVIES.items():
            for entry in movies:
                # each entry is (title, score, language)
                title = entry[0]
                score = entry[1]
...
def featured():
    ...
    try:
        top_by_lang = {}
        for genre, movies in MOCK_MOVIES.items():
            for title, score, language in movies:
                if language not in top_by_lang or score > top_by_lang[language]['score']:
                    top_by_lang[language] = {'title': title, 'score': score, 'language': language, 'genre': genre}
            ...
            for genre, movies in MOCK_MOVIES.items():
                for title, score, language in movies:
                    if title and title.lower() == ct.lower():
                        curated.append({'title': title, 'score': score, 'language': language, 'genre': genre})
                        seen.add(title)
                        found = True
                        break
                if found:
